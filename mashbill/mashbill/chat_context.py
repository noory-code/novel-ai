...
WRITE_PLAYBOOK = (
    ...
    "after another step), CONNECT it in the same confirmed action: call "
    "create_edge with the parent as source and the new node as target — the "
    "user's one yes covers the node AND its line; a registered node must never "
    "float unconnected. "
