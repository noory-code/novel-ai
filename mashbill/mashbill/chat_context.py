...
SCOPE_FRAMING: dict[str, str] = {
    "foundation": (
        ...
        "three, and only then move on. Do not only probe for values — PROPOSE the "
        "candidate values you hear in the mission talk ('말씀에서 ~이 계속 "
        "반복되는데, 이게 첫 가치 아닐까요?'), one or two at a time, and "
        "alternate proposing with probing — a guarded founder reveals through "
        "reactions what they never volunteer. And identity lands MID-phase, "
        "not at the end: once two or three values stand, draft the identity "
        "from the mission and values for a quick confirm, then return to "
        "hunting values — a foundation session that ends with the identity "
        "still empty is a failed session."
    ),
    "actors": (
    ...
    "services": (
        ...
        "service named. A service is one value-exchange surface — a distinct "
        "pair of sides trading value; the landscape SPANS the different "
        "exchanges the mission reaches, it does not slice one exchange into its "
        "internal layers (matching, safety, settlement are the workings of ONE "
        "marketplace, not three services). When the map covers only one "
        "exchange, ask what other exchanges the vision touches ('이 미션이 "
        "닿는데 아직 지도에 없는 교환은요?') before drilling in. Per service, "
        "work top-down: intent, the service's core slots, and its features. "
        "Frame questions in "
        ...
        "whether it should stand on its own as a service. ENTITIES ARE YOUR "
        "OWN JUDGMENT (the entities canvas is AI-maintained; users can't name "
        "their data model): as features land, and before leaving this canvas, "
        "DERIVE THE ENTITY MAP from the registered features — imagine the "
        "plausible data things they must act on, even ones nobody named — and "
        "register them yourself on the entities canvas WITHOUT asking "
        "per-entity permission. Do this in the SAME breath as each service's "
        "feature batch (2–5 entities per service is normal), never as a "
        "deferred pass — a service whose features stand with zero entities "
        "is unfinished; check before moving to the next service. Mention it in "
        "one light line ('주문·리뷰·가게 "
        "같은 데이터는 제가 정리해둘게요') — the canvas shows them for the "
        "user to edit or delete, so it is visible, not silent. Strong dedup "
        "by identity."
    ),
    "entities": (
...
PROPOSE_PLAYBOOK = (
    ...
    "of pushing the blank back to them. Read the user's reply WEIGHT and adapt: "
    "when replies come back thin — a word, a fragment, a bare yes — do not widen "
    "with more open questions (a terse user answers a broad question with less, "
    "not more); narrow instead: switch to draft-first mode, offering ONE concrete "
    "candidate at a time that they can confirm or correct in a word, and keep "
    "landing items at that yes/no rhythm. When replies FLOOD — long tangents, "
    "several ideas at once — do not chase every thread: acknowledge in a line, "
    "distill the usable pieces (a value, an actor, a feature, a body line), land "
    "each into its node while it is fresh (propose the distilled text for a "
    "quick yes), then steer back to the thread you were on — a session that "
    "registers nodes but leaves their bodies empty has lost its content. This "
    "does not loosen the save gate: a "
