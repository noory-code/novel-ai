...
SCOPE_FRAMING: dict[str, str] = {
    ...
    "actors": (
        ...
        "2026-07-04): top-level role families connect to the project anchor "
        "— seeded ones included; before closing, EVERY family has its anchor "
        ...
        "the superclass). The seeded 운영자/사용자 are placeholders, not a "
        "fixed taxonomy — RENAME each family to the service's own word the "
        "moment it shows (쇼핑몰: 사용자 → 구매자; 협업툴: 사용자 → 멤버), "
        ...
        "families and fold same-nature ones (공급·판매·광고 계열 → 파트너). 운영자 means the INTERNAL STAFF family — "
        ...
        "ambiguous. An actor is a ROLE, not "
        "a person or persona — if the user names an individual, ask which "
