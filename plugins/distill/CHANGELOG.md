...
- Added official Codex plugin metadata and the safe `manage_entry` MCP tool for
  promote, demote, and delete operations that do not require MCP Sampling.
- Added the repository-root Gemini extension integration with an explicit
  non-Sampling Distill tool allow-list.

### Changed

- Moved the package to `plugins/distill/` and made its MCP launch path resolve
  from each host's installed plugin directory.
- Separated Claude-only automatic hooks and Sampling-backed skills from the
  cross-host skill set. Codex and Gemini receive init, recall, profile, digest,
  direct store, and entry management without a false automatic-extraction claim.
- Removed the production-exposed `test_raw_sampling` diagnostic tool.
