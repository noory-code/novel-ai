...
> Local knowledge MCP server with Claude-specific automatic extraction.
...
- **Claude-only MCP workflows** (`learn`, `ingest`, `memory(action="crystallize")`):
  use MCP Sampling through the user's Claude subscription.
- **Cross-host MCP tools** (`init`, `recall`, `profile`, `digest`, `store`,
  `manage_entry`): require no server-initiated sampling.
...
| `manage_entry(action, id)` | Cross-host promote/demote/delete without sampling |
