...
Distill runs in the background. At the end of each Claude Code session, a hook reads your conversation and extracts key insights — decisions, patterns, solutions — using Claude's own intelligence via MCP Sampling (no API key needed). These insights are stored in a local SQLite database, indexed by both full-text search and semantic embeddings.

When you start a new task, `recall("your question")` searches across all three scopes (global → workspace → project) and returns the most relevant knowledge from your history. Over time, Distill builds a knowledge base that makes you — and Claude — smarter in every new context.
...
cd novel-ai/plugins/distill && uv sync
```
...
```json
{
  "mcpServers": {
    "distill": {
      "command": "uv",
      "args": ["run", "--directory", "/absolute/path/to/novel-ai/plugins/distill", "python", "-m", "distill"]
    }
  }
}
```

> **Note:** Manual installation does not activate hooks automatically. To enable background extraction, add the hooks from `claude-hooks/hooks.json` to your Claude Code settings manually.

Codex and Gemini CLI receive Distill's host-neutral local store and recall MCP
tools. Automatic conversation extraction remains Claude-specific because it
uses Claude Code's transcript hook and `claude -p`. See the
[host support matrix](../../docs/HOST_SUPPORT.md) for the exact tool boundary.
...
Core tools: `init`, `recall`, `store`, `profile`, `digest`, `manage_entry`.
Claude Code additionally exposes the Sampling-backed `learn`, `ingest`,
`memory`, and crystallization workflows.
...
**Q: My Claude session-end hook isn't extracting knowledge. How do I debug?**
Run `distill-profile` and inspect the hook status, then check the temporary log
path reported by the hook. Automatic extraction requires the `claude` command
on `PATH` and a working Claude Code login.
