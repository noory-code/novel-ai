...
import { wrapSamplingError } from "../src/extractor/sampling-error.ts";
...
describe("callLlm", () => {
  ...
  it("wraps 'not supported' error with user-friendly message", async () => {
    const { server } = createMockServer({
    ...
    await assert.rejects(
      ...
      (err: Error) => {
        ...
        assert.ok(err.message.includes("noory-code/novel-ai"));
        return true;
      },
    );
  });

  it("wraps 'Method not found' error with user-friendly message", async () => {
    const { server } = createMockServer({
...
describe("wrapSamplingError", () => {
  it("wraps 'not supported' error", () => {
    const err = wrapSamplingError(new Error("Method not supported"));
    assert.ok(err.message.includes("MCP Sampling is not supported"));
  });

  it("wraps 'Method not found' error", () => {
    const err = wrapSamplingError(new Error("Method not found: sampling/createMessage"));
    assert.ok(err.message.includes("MCP Sampling is not supported"));
  });

  it("wraps 'sampling' keyword error", () => {
    const err = wrapSamplingError(new Error("sampling request failed"));
    assert.ok(err.message.includes("MCP Sampling is not supported"));
  });

  it("passes through unrelated errors unchanged", () => {
    const original = new Error("network timeout");
    const err = wrapSamplingError(original);
    assert.equal(err, original);
  });

  it("converts non-Error to Error", () => {
    const err = wrapSamplingError("string error");
