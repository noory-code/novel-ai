...
- Added an official Codex plugin manifest and host-neutral MCP launch contract.
- Added current skills for structured project creation, project reading, and
  immutable format-F publication.

### Changed

- Moved the package to the shared `plugins/mashbill/` marketplace layout used
  by Claude Code and Codex.
- Rewrote the public package guide and SessionStart context around the actual
  headless engine and `.noory/novel/` data contract.
- Removed the obsolete React Flow sketch skills and the maintainer-only UI
  verifier agent, which depended on a private viewer and a local fixture.
- Removed the obsolete viewer pre-commit hook and its private-viewer checks from
  the public plugin.
