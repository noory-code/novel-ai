...
- **What:** `novel-ai` is one public repository and distribution source. Claude
  Code and Codex each install Mashbill, Solera, Proof, and Distill as four
  independent plugins from their official marketplace manifests. Gemini CLI
  installs one root extension that registers the same four packages as four
  stdio MCP servers. Distill's Claude-only Sampling and transcript-hook features
  are separated from the cross-host local-store surface.
- **Why:** One install source makes the suite discoverable and maintainable,
  while separate plugin and process boundaries preserve dependency isolation,
  fault containment, tool namespaces, and independent use. A monolithic MCP
  would erase those boundaries without reducing the number of domain contracts.
- **Alternatives:** One monolithic MCP server was rejected because every host
  would start all dependencies and share one failure domain. Four separate
  Gemini extensions were rejected because the user wants a single suite install
  and the official Gemini manifest already supports multiple MCP servers.
- **Approval:** Accepted by user through the explicit request to support the
  official Claude Code and Codex structures and provide an informed Gemini MCP
  design, 2026-07-10.
- **Spec impact:** Distribution and capability SSOT is
  `docs/HOST_SUPPORT.md`; runtime data and package independence are unchanged.
...
- **Spec impact:** [`SPEC.md` §Pan and select](./SPEC.md#pan-and-select)
  ...
  [`mashbill/CLAUDE.md`](../CONTRIBUTOR_GUIDE.md) anti-patterns — new row banning
...
- **Spec impact:** [`SPEC.md` §Hover behaviour](./SPEC.md#hover-behaviour)
  ...
  rewritten to mirror the vendor CSS exactly. [`mashbill/CLAUDE.md`](../CONTRIBUTOR_GUIDE.md)
...
- **What:** Add a new pre-action gate at position 0 (before the
  existing Gate 1) in [`mashbill/CLAUDE.md`](../CONTRIBUTOR_GUIDE.md). The gate
