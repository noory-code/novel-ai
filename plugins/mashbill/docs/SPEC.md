...
> **동작 RULE 정본은 repository root** (`D-2026-06-28-B`) — [`docs/specs/canvas-behavior.md`](https://github.com/noory-code/novel-ai/blob/main/docs/specs/canvas-behavior.md)
> · [`edges.md`](https://github.com/noory-code/novel-ai/blob/main/docs/specs/edges.md) · [`kinds-fields.md`](https://github.com/noory-code/novel-ai/blob/main/docs/specs/kinds-fields.md).
...
- **Add a Project** (v0.34.0, D-2026-05-31-N) — the sidebar/empty-state
  ...
  project lands in its most-recently-updated one (no duplicate). A folder
  row that holds no project also offers "+ New folder": it asks for a name
  and creates that folder under the row, which can then be picked like any
  other. A row that already holds a project offers only Open — a new project
  is never nested inside an existing one (2026-06-21).
...
**Identity boundary (D-2026-08-18-F).** Identity sets the attitude and way a
service consistently behaves while it is designed, built, and presented to
people. It applies continuously; core values decide priority only when choices
conflict and override identity when the two clash. A short directive such as
"Bright and clear" is a valid name when its summary and description make the
expected actions concrete. AI drafts identity guidelines from the mission and
core values, and the person refines and confirms them. The coach does not impose
a count or fixed voice, energy, or speech-style facets.

**Foundation guidance across every chat (D-2026-08-18-E/F).** Every chat scope,
including the cross-canvas Project scope, receives the mission, core-value, and
identity definitions above in its system prompt. Every in-app turn receives a
full, current snapshot of the Mission, Core value, and Identity nodes from the
Foundation canvas. Every coach checks its proposals and replies against those
current nodes. This snapshot comes from the engine's existing context provider
and updates on the next turn after the canvas changes. An empty or unreadable
Foundation contributes no content; the engine never fills the gap. The external
agent receives the same system framing and per-turn context through
`get_viewer_context`. Detailed discriminator questions remain available through
`get_design_principles`, but the common definitions and current project facts
never depend on the model choosing to call that tool.
...
| **Context injection (D-2026-06-15-A, D-2026-08-18-E, in-app)** | Each turn carries the current Foundation, the active canvas, and the live node selection. The engine reads all Mission, Core value, and Identity text directly from the Foundation canvas and prepends it as `[Project foundation]`; this applies to every scope, including `project`, and returns nothing when Foundation is empty or unreadable. The viewer sends `selection` (`[{id, kind, label}]`) on `POST /api/chat/send`; the engine adds a `[Novel context] Active canvas: <scope>. Selected (N): …` preamble so "fix this" resolves against the selection. The selection is read from the **active surface** — including the service-detail canvas — and is capped at 20 detailed nodes. Every send rebuilds the context, so a Foundation edit reaches the next turn. (CHAT_ARCH.md Layer 2.) |
...
| **Conversation persistence (D-2026-06-26-B)** | Chat conversations persist to disk as **one append-only log per scope** at `.noory/novel/chat/<scope>.json` (parametric scopes sanitise `:`→`__`, e.g. `service:abc`→`service__abc.json`). The **engine is the sole writer**, appending the user message when a turn is sent and the assistant message on `turn_complete`, via the atomic `storage._write_json`. So a project's conversations travel with the project, restore on another machine, and **survive an engine restart** (the failure that motivated this — in-memory chat died on app restart). The dock lists saved conversations per canvas/service (`GET /api/chat/conversations`) and reopens one (`GET /api/chat/conversations/{scope}` → hydrate the scope's message bucket). When a fresh CLI session receives the saved transcript, the header tells it to preserve facts and decisions without imitating earlier assistant wording or style (D-2026-08-18-A); the stored JSON itself is never rewritten. `title` = first user message (≤60 chars), set once. **Scope:** one conversation per scope (mirrors the one-session-per-scope registry). **Continuing a reopened conversation:** reopening pins the dock to that conversation's scope, so the next message is appended to that conversation — not to the canvas that is open — and the scope switcher shows that conversation's name while pinned. Moving to another canvas or picking a scope unpins it. **Out of scope (v1):** multiple conversations per scope, CLI `--session-id` revival (reopening restores the **transcript for reading**, not the live CLI session — continuing starts a fresh turn), orphan pruning on service deletion, search/rename/delete-from-UI. |
| **Per-canvas framing + response language + hallucination guard (D-2026-06-15-C → D-2026-08-18-F)** | Each turn delivers a canvas-appropriate **system framing**, a universal **response-language rule**, a constant **hallucination guard**, and the shared Mission/Core value/Identity definitions as an **authoritative system prompt**, not glued into the user message where the model reads it as mere conversation (Lever 2, `docs/idea/chat/01-levers.md`). The language rule, guard, and Foundation definitions apply to **every** scope, including `project`. The base scope maps to its VISION phase, and the framing is the canvas's full coaching playbook: Foundation → Discovery, Actors / Services → Planning, Feature → Execution. A shared coach tone prefixes every canvas framing. The `project` scope gets the three universal blocks but no tone or canvas framing. Delivery is per-CLI: **claude** uses `--append-system-prompt`; CLIs without that flag receive the system prompt before the user message. `build_system_prompt` remains the one code owner and is shared with the MCP path. Detailed discriminator questions remain in `get_design_principles`, not duplicated into the prompt. |
...
| **MCP-path context (D-2026-06-15-D, D-2026-08-18-E, primary path)** | The external agent reads the viewer's live context through `get_viewer_context(project_path)` → `{active_canvas, selection, framing, context, updated_at, stale, has_viewer}`. `framing` is the same system prompt the in-app path uses. `context` is the same per-turn block the in-app path builds, including the current full Foundation and the active-canvas context. The viewer and HTTP sidecar still share the live scope through the project-keyed temporary file. If that report is older than the 90-second limit, the tool returns no active canvas, selection, framing, or context. Every live call rebuilds `context` from the current project files, so a Foundation edit reaches the next read. **Multi-viewer:** last-writer-wins, disambiguated by `updated_at`. |
...
| **MCP wiring** | Novel's MCP server is registered with each CLI separately (Track 2.5: `~/.claude.json` / `~/.codex/config.toml` / `~/.gemini/settings.json`). The canvas ↔ chat feedback loop runs through MCP. **Registered command shape (D-2026-06-14-A):** in a dev checkout, `uv run --directory <src> python -m mashbill`; in the bundled `.app` (PyInstaller-frozen), `<bundled binary> --mcp-stdio` (`command = sys.executable`) — the dev `uv run` command points at the ephemeral `_MEIxxxx` extraction dir and times out, so the frozen path registers the stable bundled binary in its stdio-MCP mode instead. **In-app chat does NOT use this global registration (D-2026-06-26-E, D-2026-07-21-B):** each provider injects the *running build's own* Novel stdio server with the same frozen/dev command shape. Claude uses `--mcp-config` plus `--strict-mcp-config`; Codex uses `-c mcp_servers.mashbill.command=...` and `.args=...` plus `--ignore-user-config`. Both therefore carry this build's Novel tools rather than a drift-prone global pointer. Codex's headless turn runs with `--ask-for-approval never --sandbox read-only`, so Mashbill MCP writes do not dead-end on the provider's `stdin=DEVNULL`, while Codex's built-in shell stays read-only. The global registration remains for the user's *external* agent (the MCP-first primary path). |
...
learn it in place. Concept SSOT = [`concepts/kinds.md`](https://github.com/noory-code/novel-ai/blob/main/docs/concepts/kinds.md)
...
| Aspect | Behaviour |
|---|---|
| **Source of edges** | An edge is governed by its **definition** (its `relation` + payload), **not by who draws it** — the former "all edges are user-drawn / no auto-emit" ban is removed (**D-2026-06-17-J**). Edges may be **user-drawn OR AI-proposed/drawn**; the AI maintains relationships on **AI-maintained canvases** (e.g. Entities, D-2026-06-17-I), and the user can always edit / delete any edge. A canvas may still be user-draw-only by its own spec (Foundation / Actors currently are; Services is user-draw-only EXCEPT the containment line of a coach-registered node — `create_edge` draws service→feature in the same confirmed action as the `create_node` it belongs to, D-2026-07-02-J — a per-canvas choice, not a global law). **Never emit a *meaningless* or *silently-uneditable* line** — that specific harm (v0.13.2) stays banned. (Migration: read-side v0.26.0 auto-creates directed edges from legacy ``parent_id`` — one-time per canvas, D-2026-05-25-A.) |
...
- `identity` — `summary` + `description` (**D-2026-07-03-S / D-2026-07-04-B /
  D-2026-08-18-F**): `label` is the short directive, `summary` states the intended
  behavior in one line, and `description` lists the concrete actions. Legacy
  `body` content folds into `description` on read and is not shown in the
  inspector. Earlier **v0.43.2, D-2026-06-06-B** had already removed `do` /
...
Selecting a node fills the sidebar panel with its inspector, covering the
palette; deselecting returns the palette (D-2026-06-21-Q). Every canvas,
the Feature canvas included, behaves this way — the Feature canvas's
always-on read-only subject-service inspector is retired. When the sidebar
is collapsed, the inspector falls back to an overlay on the canvas's right
edge.

| Aspect | Behaviour |
|---|---|
| **Trigger** | Single click on a node (any kind except — TBD anchor). |
| **Width** | The sidebar panel's width; the user resizes it with the panel separator. There is no narrow/wide toggle (removed by D-2026-06-21-Q). |
| **Sections (Foundation)** | Header (kind, delete, close) → `_md_warnings` list (if any) → Label field → Typed-field forms per kind → MD-template editor (edit / split / preview). |
| **Close** | × button in header, or click empty canvas (pane click clears selection). |
| **Read-only mode (D-2026-06-15-O)** | `KindInspector` / `BaseInspector` accept `readOnly`: the label input is non-editable, and the delete / close / publish / published-versions / details-MD affordances are hidden. The per-kind body renders its fields read-only (Service shows its 5-field question-titled summary — 누가 참여하나? / 왜 필요한가? / 뭐가 좋아지나? / 뭘 양보 못 하나? / 어떤 결로 다가가나?, D-2026-06-17-B). Its Feature-canvas fallback use is unwired since D-2026-06-21-Q. |
...
**Actors structure lines point outward (D-2026-07-04-K).** The Actors canvas
no longer converges: its anchor spokes and inheritance edges render from the
anchor outward like a tree (`anchorArrowMode="diverge-structure"`), and its
value-exchange edges keep their own giver→receiver direction. The converge
rule below now applies to Foundation only.
...
> **Retired by D-2026-06-21-Q.** The Feature canvas now behaves like every
> other canvas: the sidebar shows the palette until a node is selected, then
> that node's inspector (see [Inspector](#inspector-right-panel)). The
> three-state rule below describes the retired behaviour; its host component
> remains in the code but is not wired from App.
