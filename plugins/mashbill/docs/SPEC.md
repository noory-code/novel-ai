...
| **Per-canvas framing + response language + hallucination guard (D-2026-06-15-C → D-2026-08-18-A)** | Each turn delivers a canvas-appropriate **system framing**, a universal **response-language rule**, and a constant **hallucination guard** as an **authoritative system prompt**, not glued into the user message where the model reads it as mere conversation (Lever 2, `docs/idea/chat/01-levers.md`). The language rule applies to **every** scope, including `project`: reply in the user's language; for Korean, use ordinary conversational Korean with correct particles and spacing, keep standard technical terms in their normal Korean form, and neither translate instruction metaphors nor invent new ones. The guard ("ground every statement in the given context, read the canvas with your mashbill MCP tools when you don't know, ask otherwise — never invent project details; resolve 'this' to the selected node") also ships on every scope. The base scope maps to its VISION phase, and the framing is the canvas's **full coaching playbook** (D-2026-06-24-E): Foundation → Discovery (the mission / core-value / identity interview), Actors / Services → Planning (role-family elicitation / the 5-slot JTBD interview + promotion test), Feature → Execution (actor-anchored main flow plus immediate branches and an implementation boundary). A shared **coach tone** (gentle, one-question-at-a-time, follow the user's words) prefixes every canvas framing. Content SSOT = `docs/concepts/ai-collaboration.md` §0–§2; the English runtime framing states behavior directly and does not provide figurative shorthand for the reply to copy. The `project` scope gets the response-language rule and guard only (no tone, no canvas framing). Delivery is per-CLI: **claude** uses the native `--append-system-prompt` flag; CLIs without that flag receive the system prompt before the user message. The engine assembles the user message as **canvas map → cross-canvas registry → selected-node details → user text** (framing lives in the system prompt, not the message). `build_system_prompt` is code constants (`RESPONSE_LANGUAGE` + `HALLUCINATION_GUARD` + canvas playbooks + `SCOPE_FRAMING`), not `.noory/`-editable, SSOT in `mashbill/chat_context.py`, and is **shared with the MCP path** (D-2026-06-24-F). (CHAT_ARCH.md Layer 3.) |
