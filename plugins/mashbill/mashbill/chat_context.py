...
SCOPE_FRAMING: dict[str, str] = {
    ...
    "services": (
        ...
        "describes HOW one value exchange on the Actors canvas happens. Derive "
        "service candidates from the EXCHANGE LINES; several lines may belong to "
        "one service. Set each service's participants from the actors at the ends "
        "of those lines with set_node_references ref_actor_ids. Cover the distinct "
        "exchanges reached by the mission. Do not split one exchange into internal "
        "processes: matching, safety, and settlement can all be parts of one "
        "marketplace. If only one exchange is listed, ask what other exchanges the "
        "mission reaches. "
        ...
        "doer with set_node_references ref_actor_ids, choosing from its service's "
        "participants. A feature is what ONE person does inside a service. If a "
        "candidate names an audience, market, or revenue line, ask whether it is a "
        "separate service. "
        "Before closing, group services into categories. A category IS a platform "
        "(플랫폼: where the product meets its people, such as 고객 앱 or 사장님 웹). "
        "Set each platform's ref_actor_ids to the role families who use it. A "
        "service's participants must stay within its platform's participants. "
