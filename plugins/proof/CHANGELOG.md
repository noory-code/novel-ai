...
- Added a FastMCP server for recording, listing, reading, filtering, and
  checking immutable decisions.
- Added official Codex plugin metadata and a Gemini-compatible stdio entry.

### Changed

- Moved the package to `plugins/proof/` and converted the skills from
  Claude-specific shell commands to the same MCP operations used by every host.
