...
- Added a FastMCP server for planning, execution, status, import, re-pin,
  retrospective, and feedback operations on the existing file-based core.
- Added the built-in `proof:<topic>` gate, which checks Proof's public
  append-only file contract without importing Proof or requiring its CLI on
  `PATH`.
- Added an official Codex plugin manifest and Gemini-compatible stdio entry.

### Changed

- Moved the package to `plugins/solera/` and converted every shipped skill from
  Claude-specific shell commands to the host-neutral MCP tools.
- Corrected imported format-F paths in the public skills to
  `.noory/solera/specs/{label}/service/`.
