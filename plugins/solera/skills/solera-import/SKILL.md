...
Call Solera MCP `import_spec` with the current workspace as `project_root`, the
absolute or workspace-relative release directory as `source`, and the chosen
`label`.
...
The imported release lands at `.noory/solera/specs/<label>/`. Open
`.noory/solera/specs/<label>/service/design/features/` to see which features it defines. Each
