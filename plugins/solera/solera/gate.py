"""The gate-runner — Solera's deterministic verification (V) step.

A gate is a single command. It is tokenised with :func:`shlex.split` and run
with ``shell=False`` so the verdict is deterministic and never depends on a
shell being present or on shell expansion. The rule is exactly: exit code 0
passes, anything else fails. A gate that cannot be *started* (empty command,
binary not found) is a :class:`GateResult` with ``passed=False`` and no exit
code — never a silent pass.

This is intentionally not an LLM step. The harness must be able to trust the
verdict, so it is a plain subprocess with a fixed pass rule.
"""

from __future__ import annotations

import shlex
import subprocess
from dataclasses import dataclass
from pathlib import Path
from typing import Any

import yaml

from .errors import GateError
from .formats import WorkItem

DEFAULT_TIMEOUT_SECONDS = 120.0
PROOF_GATE_PREFIX = "proof:"


def _as_text(value: object) -> str:
    """Coerce captured stream output (str | bytes | None) to text."""
    if value is None:
        return ""
    if isinstance(value, bytes):
        return value.decode(errors="replace")
    return str(value)


@dataclass(frozen=True)
class GateResult:
    """The outcome of running one gate, with evidence."""

    command: str
    passed: bool
    exit_code: int | None
    stdout: str
    stderr: str
    timed_out: bool


def run_gate(
    command: str,
    *,
    cwd: Path,
    timeout: float = DEFAULT_TIMEOUT_SECONDS,
) -> GateResult:
    """Run ``command`` in ``cwd`` and return a :class:`GateResult`.

    Raises :class:`GateError` only when the command string is empty — there is
    nothing to run. Every other failure mode (non-zero exit, timeout, binary
    not found) is reported as ``passed=False`` so the supervisor can branch on
    it without handling exceptions.
    """
    argv = shlex.split(command)
    if not argv:
        raise GateError("empty gate command: nothing to run")

    try:
        proc = subprocess.run(
            argv,
            cwd=str(cwd),
            capture_output=True,
            text=True,
            timeout=timeout,
            shell=False,
        )
    except subprocess.TimeoutExpired as exc:
        return GateResult(
            command=command,
            passed=False,
            exit_code=None,
            stdout=_as_text(exc.stdout),
            stderr=_as_text(exc.stderr) + f"\ngate timed out after {timeout}s",
            timed_out=True,
        )
    except (FileNotFoundError, OSError) as exc:
        return GateResult(
            command=command,
            passed=False,
            exit_code=None,
            stdout="",
            stderr=f"could not run gate: {exc}",
            timed_out=False,
        )

    return GateResult(
        command=command,
        passed=proc.returncode == 0,
        exit_code=proc.returncode,
        stdout=proc.stdout,
        stderr=proc.stderr,
        timed_out=False,
    )


def run_proof_gate(topic: str, *, cwd: Path) -> GateResult:
    """Check Proof's plain-file contract without importing the sibling package.

    ``proof:<topic>`` is Solera's built-in cross-plugin gate. It passes when an
    accepted, non-superseded decision under ``.noory/proof`` includes the topic
    in its ``about`` list. Malformed decision frontmatter fails closed.
    """
    command = f"{PROOF_GATE_PREFIX}{topic}"
    if not topic.strip():
        raise GateError("proof gate topic must not be empty")

    proof_root = cwd / ".noory" / "proof"
    accepted: dict[str, dict[str, Any]] = {}
    try:
        paths = sorted(proof_root.glob("PROOF-*.md")) if proof_root.is_dir() else []
        for path in paths:
            lines = path.read_text(encoding="utf-8").splitlines()
            if not lines or lines[0].strip() != "---":
                raise ValueError(f"{path.name}: missing YAML frontmatter")
            try:
                end = next(
                    index for index, line in enumerate(lines[1:], 1) if line.strip() == "---"
                )
            except StopIteration as exc:
                raise ValueError(f"{path.name}: missing closing frontmatter fence") from exc
            data = yaml.safe_load("\n".join(lines[1:end])) or {}
            if not isinstance(data, dict):
                raise ValueError(f"{path.name}: frontmatter must be a mapping")
            if data.get("status") == "accepted":
                accepted[path.stem] = data
    except (OSError, ValueError, yaml.YAMLError) as exc:
        return GateResult(command, False, 1, "", f"could not read Proof log: {exc}", False)

    retired = {
        decision.get("supersedes")
        for decision in accepted.values()
        if isinstance(decision.get("supersedes"), str)
    }
    matches = [
        decision_id
        for decision_id, decision in accepted.items()
        if decision_id not in retired
        and isinstance(decision.get("about"), list)
        and topic in decision["about"]
    ]
    if matches:
        return GateResult(command, True, 0, "\n".join(matches) + "\n", "", False)
    return GateResult(
        command,
        False,
        1,
        "",
        f"no in-force Proof decision is tagged with {topic!r}",
        False,
    )


def run_item_gate(
    item: WorkItem,
    *,
    cwd: Path,
    timeout: float = DEFAULT_TIMEOUT_SECONDS,
) -> GateResult:
    """Run the gate declared on a leaf ``item``.

    Only a leaf carries a gate. An item with no gate cannot be auto-verified, so
    this raises :class:`GateError` rather than passing it silently — the caller
    must escalate such an item to a human.
    """
    if not item.gate.strip():
        raise GateError(f"item {item.id} has no gate to run")
    if item.gate.startswith(PROOF_GATE_PREFIX):
        return run_proof_gate(item.gate.removeprefix(PROOF_GATE_PREFIX), cwd=cwd)
    return run_gate(item.gate, cwd=cwd, timeout=timeout)
