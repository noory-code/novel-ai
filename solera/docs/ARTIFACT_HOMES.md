...
| **Process artifacts** (scratch output on the way: a draft, a screenshot, a dump) | `stories/{id}/artifacts/` | Staging only. Tagged, not versioned (below). |

## Staging convention (`stories/{id}/artifacts/`)

Process artifacts stage under the Story that produced them and carry **tags**,
...
Tags live in an optional manifest, `stories/{id}/artifacts/MANIFEST.md`:
...
`solera.artifacts.find_misplaced_artifacts(ws)` scans every Story's
