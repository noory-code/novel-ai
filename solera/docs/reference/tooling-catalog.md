...
| `{project_slug}` | `{project_name}` normalized: (1) unicode-NFC, (2) lowercase, (3) replace `_` and whitespace (`\s+`) with single `-`, (4) strip any character not matching `[a-z0-9-]`, (5) collapse consecutive `-`, (6) strip leading/trailing `-`. Result must match `^[a-z][a-z0-9-]*[a-z0-9]$` and be 3–50 chars. If it doesn't (e.g. empty after stripping non-ASCII like `노리 AI` → ``), halt Step 6 and ask the user for a slug via AskUserQuestion with this context: `"Project name '{project_name}' does not normalise to a valid kebab-case slug (result: '{attempted_slug}'). Slugs go into file names and trigger phrases, so ASCII-only is required. Provide a slug (lowercase letters / digits / hyphens, 3–50 chars):"`. | `billing-api-service-v2` from `Billing_API Service v2` |
