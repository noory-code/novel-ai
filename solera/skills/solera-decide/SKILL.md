...
uv run --directory "${CLAUDE_PLUGIN_ROOT}" solera --root "$PWD" \
  ...
  --gate 'proof --root "$PWD" check --about auth-stack'
```
...
3. The human chooses and records the decision in proof:

   ```bash
   proof --root "$PWD" record "Use Postgres" --status accepted \
     --about auth-stack \
     --body "$(cat <<'EOF'
   ## Context
   ...
...
- **You surface; the human decides.** Never run `proof record --status accepted`
