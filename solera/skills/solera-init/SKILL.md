...
- [ ] **Load the catalog**. Read `${CLAUDE_PLUGIN_ROOT}/docs/reference/tooling-catalog.md`. If the file is missing, unreadable, or obviously truncated (no `## How Step 6 uses this catalog` section): halt Step 6 with the error `"Tooling catalog is missing or corrupt at docs/reference/tooling-catalog.md. Reinstall the solera plugin or restore the file. Step 6 cannot continue."`. Do **not** fall back to an inline catalog copy or skip silently.
- [ ] **Catalog integrity check — unique candidate names**: parse all candidate section headers that match `^### \d+\. \S+ (agent|skill) +\((FULLY SPECIFIED|placeholder — coming soon)\)$`. Extract the name token. If any name appears twice (across all `project.type` sections), halt Step 6 with the error `"Tooling catalog has duplicate candidate name '{name}'. Candidate names must be globally unique across the catalog. Fix docs/reference/tooling-catalog.md."`. This runs before any user prompt.
