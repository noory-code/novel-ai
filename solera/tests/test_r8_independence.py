...
`solera_mcp/`; anything rooted at a banned name fails. Also bans `src-tauri`
path literals (a runtime reach into the shell). Solera's own React `viewer/`
and `vscode-extension/` are separate build artefacts; this guard covers the
Python MCP package, which must not import the Plot app or a sibling plugin.
"""
...
PKG = Path(__file__).resolve().parent.parent / "solera_mcp"
