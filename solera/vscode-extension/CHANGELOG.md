...
Initial Marketplace release. Ships as part of unified Solera v1.0.0 — same version as the Claude Code plugin, same `.solera/` data format, same MCP server (bundled here via `../viewer/dist/` and `../solera_mcp/` from the parent plugin directory).
...
- Detects `.solera/` (Solera v1) or `workspace/` (pre-v1, with a deprecation info notification suggesting `/solera-migrate-workspace-to-dotsolera` in Claude Code).
...
- Server lifecycle: SIGTERM with 5s grace period, then SIGKILL on extension deactivation. Premature child exit detected within 200ms (was 15s — fixed during v1.0 development).
